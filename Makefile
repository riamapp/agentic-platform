...
help:
	...
	@echo "  mcp-lambda-zip              - Build MCP Lambda package (mcp_lambda.zip)"
...
# Build both Lambda packages (legacy target)
lambda-zip: $(MCP_LAMBDA_ZIP)
...
# Build MCP Lambda package with Docker for Linux-compatible dependencies
# Only rebuilds if source files are newer than the zip file
$(MCP_LAMBDA_ZIP): mcp/lambda/handler.py mcp/lambda/requirements.txt
