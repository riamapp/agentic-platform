...
# Package MCP Lambda function with dependencies using Makefile (ensures Linux-compatible builds)
...
resource "aws_lambda_function" "mcp_lambda" {
  ...
  description   = <<-EOT
    Lambda function for Bedrock AgentCore Gateway that handles MCP tool invocations. 
    Implements a placeholder tool that demonstrates argument passing from the AgentCore 
    Gateway to Lambda targets.
  EOT
...
resource "aws_iam_role_policy" "mcp_lambda_agentcore_identity" {
  role = aws_iam_role.mcp_lambda_role.id
  name = "${var.app_name}-McpLambdaAgentCoreIdentityPolicy"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        Sid    = "AgentCoreIdentityDataPlaneAccess"
        Effect = "Allow"
        Action = [
      ...
      {
        Sid    = "AgentCoreIdentityControlPlaneReadAccess"
        Effect = "Allow"
        Action = [
      ...
      {
        Sid    = "AgentCoreIdentityControlPlaneAccess"
        Effect = "Allow"
        Action = [
